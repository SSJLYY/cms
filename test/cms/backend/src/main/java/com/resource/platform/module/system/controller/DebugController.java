 ...
 */
@Slf4j
@Profile("dev")  // ✅ 安全修复：仅开发环境可用，生产环境自动关闭
@Tag(name = "调试工具")
@RestController
...
public class DebugController {
     ...
     * @throws BusinessException 当密码为空时抛出
     */
    ...
    public Result<Map<String, String>> encodePassword(@RequestParam String password) {
        ...
        try {
            ...
            if (password == null || password.trim().isEmpty()) {
                ...
                throw new BusinessException(BizErrorCode.PARAM_ERROR, "密码不能为空");
            }
            
            // 检查密码长度是否合理
            if (password.length() > 100) {
                log.warn("密码编码调试失败: 密码长度过长, length={}", password.length());
                throw new BusinessException(BizErrorCode.PARAM_ERROR, "密码长度不能超过100个字符");
            }
    ...
    public Result<Map<String, Object>> verifyPassword(
        ...
        try {
            ...
            if (rawPassword == null || rawPassword.trim().isEmpty()) {
                ...
                throw new BusinessException(BizErrorCode.PARAM_ERROR, "原始密码不能为空");
            }
            
            // 检查编码密码是否为空
            if (encodedPassword == null || encodedPassword.trim().isEmpty()) {
                log.warn("密码验证调试失败: 编码密码为空");
                throw new BusinessException(BizErrorCode.PARAM_ERROR, "编码密码不能为空");
            }
