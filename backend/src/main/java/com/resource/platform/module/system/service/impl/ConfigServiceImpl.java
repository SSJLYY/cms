...
public class ConfigServiceImpl implements ConfigService {
    ...
    public List<SystemConfig> getConfigsByCategory(String category) {
        ...
        if (category == null || category.trim().isEmpty()) {
            log.warn("配置分类参数为空");
            throw new BusinessException(BizErrorCode.PARAM_ERROR, "配置分类不能为空");
        }
    ...
    public SystemConfig getConfigByKey(String configKey) {
        ...
        if (configKey == null || configKey.trim().isEmpty()) {
            log.warn("配置键参数为空");
            throw new BusinessException(BizErrorCode.PARAM_ERROR, "配置键不能为空");
        }
    ...
    public void updateConfig(String configKey, String configValue) {
        ...
        if (rows > 0) {
            ...
            throw new BusinessException(BizErrorCode.CONFIG_UPDATE_FAILED);
        }
    }
    ...
    public void resetConfig(String configKey) {
        ...
        if (rows > 0) {
            ...
            throw new BusinessException(BizErrorCode.CONFIG_RESET_FAILED);
        }
    }
    ...
    public boolean testEmailConfig(Map<String, String> emailConfig) {
        ...
        try {
            ...
            // 步骤2：使用 JavaMail Transport 测试 SMTP 连接
            ...
            // 587/465 端口启用 STARTTLS/SSL
            String portNum = port.trim();
            if ("587".equals(portNum)) {
                props.put("mail.smtp.starttls.enable", "true");
            } else if ("465".equals(portNum)) {
            ...
            try (javax.mail.Transport transport = session.getTransport("smtp")) {
                transport.connect(host.trim(),
            ...
            // 步骤3：记录测试结果
            log.info("邮件配置测试成功: host={}, port={}, username={}", host, port, username);
            return true;
    ...
    /**
     * 测试阿里云OSS配置
     * 通过构造 OSS endpoint URL 并发送 HEAD 请求验证可达性（SDK 尚未引入时的轻量验证）
     */
    private boolean testAliyunOSSConfig(Map<String, String> config) {
        ...
        String bucketName = config.getOrDefault("bucketName", "").trim();
        if (endpoint.isEmpty() || bucketName.isEmpty()) {
            log.warn("阿里云OSS配置不完整: endpoint 或 bucketName 为空");
            return false;
        }
        try {
            ...
            conn.disconnect();
            // OSS endpoint 可达（200/403 均视为节点连通）
            boolean reachable = code == 200 || code == 403;
            log.info("阿里云OSS endpoint 可达性测试: url={}, httpCode={}, reachable={}", url, code, reachable);
            return reachable;
        } catch (Exception e) {
            log.warn("阿里云OSS endpoint 不可达: endpoint={}, error={}", endpoint, e.getMessage());
            return false;
        }
    }
    ...
    /**
     * 测试腾讯云COS配置
     * 通过 HTTP HEAD 请求验证 endpoint 可达性
     */
    private boolean testTencentCOSConfig(Map<String, String> config) {
        ...
        String bucketName = config.getOrDefault("bucketName", "").trim();
        if (endpoint.isEmpty() || bucketName.isEmpty()) {
            log.warn("腾讯云COS配置不完整: endpoint 或 bucketName 为空");
            return false;
        }
        try {
            String url = (endpoint.startsWith("http") ? endpoint : "https://" + endpoint);
            ...
            conn.disconnect();
            boolean reachable = code == 200 || code == 403;
            log.info("腾讯云COS endpoint 可达性测试: url={}, httpCode={}, reachable={}", url, code, reachable);
            return reachable;
        } catch (Exception e) {
            log.warn("腾讯云COS endpoint 不可达: endpoint={}, error={}", endpoint, e.getMessage());
            return false;
        }
    }
    ...
    private boolean testHuaweiOBSConfig(Map<String, String> config) {
        ...
        String bucketName = config.getOrDefault("bucketName", "").trim();
        if (endpoint.isEmpty() || bucketName.isEmpty()) {
            log.warn("华为云OBS配置不完整: endpoint 或 bucketName 为空");
            return false;
        }
        try {
            String url = (endpoint.startsWith("http") ? endpoint : "https://" + endpoint);
            ...
            conn.disconnect();
            boolean reachable = code == 200 || code == 403;
            log.info("华为云OBS endpoint 可达性测试: url={}, httpCode={}, reachable={}", url, code, reachable);
            return reachable;
        } catch (Exception e) {
            log.warn("华为云OBS endpoint 不可达: endpoint={}, error={}", endpoint, e.getMessage());
            return false;
        }
    }
    ...
    private boolean testAwsS3Config(Map<String, String> config) {
        ...
        String endpoint = config.getOrDefault("endpoint", "s3.amazonaws.com").trim();
        String bucketName = config.getOrDefault("bucketName", "").trim();
        if (bucketName.isEmpty()) {
            log.warn("AWS S3配置不完整: bucketName 为空");
            return false;
        }
        try {
            String url = (endpoint.startsWith("http") ? endpoint : "https://" + endpoint);
            ...
            conn.disconnect();
            boolean reachable = code == 200 || code == 403;
            log.info("AWS S3 endpoint 可达性测试: url={}, httpCode={}, reachable={}", url, code, reachable);
            return reachable;
        } catch (Exception e) {
            log.warn("AWS S3 endpoint 不可达: endpoint={}, error={}", endpoint, e.getMessage());
            return false;
        }
    }
    ...
    private boolean testLocalStorageConfig(Map<String, String> config) {
        log.debug("测试本地存储配置");
        String path = config.getOrDefault("path", "").trim();
        if (path.isEmpty()) {
            log.warn("本地存储路径为空");
            return false;
        }
        java.io.File storageDir = new java.io.File(path);
        if (!storageDir.exists()) {
            log.warn("本地存储目录不存在: path={}", path);
            return false;
        }
        if (!storageDir.isDirectory()) {
            log.warn("本地存储路径不是目录: path={}", path);
            return false;
        }
