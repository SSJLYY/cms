...
import com.resource.platform.module.promotion.mapper.LinkTypeMapper;
...
public class LinkTypeServiceImpl extends ServiceImpl<LinkTypeMapper, LinkType> implements LinkTypeService {
    ...
    public boolean save(LinkType entity) {
        ...
        if (entity == null) {
            log.warn("链接类型实体对象为空");
            throw new BusinessException(BizErrorCode.PARAM_ERROR, "链接类型数据不能为空");
        }
        ...
        if (result) {
            ...
            throw new BusinessException("链接类型保存失败");
        }
    ...
    public boolean updateById(LinkType entity) {
        ...
        if (result) {
            ...
            log.error("链接类型更新失败: id={}, name={}", 
                entity.getId(), entity.getName());
            throw new BusinessException("链接类型更新失败");
        }
    ...
    public boolean removeById(Serializable id) {
        ...
        if (id == null) {
            log.warn("链接类型ID为空");
            throw new BusinessException(BizErrorCode.PARAM_ERROR, "链接类型ID不能为空");
        }
        ...
        // 步骤3：检查关联数据
        // 注意：资源的下载链接以 JSON 形式存储在 resource.download_links 中，
        // 删除链接类型后相关资源中的链接类型引用将失效，建议前端做提示。
        // 此处记录删除操作审计日志。
        log.warn("删除链接类型: id={}, name={}，请确认相关资源中的链接类型引用已处理",
        ...
        if (result) {
            ...
            log.error("链接类型删除失败: id={}, name={}", 
                linkTypeId, existingType.getName());
            throw new BusinessException("链接类型删除失败");
        }
